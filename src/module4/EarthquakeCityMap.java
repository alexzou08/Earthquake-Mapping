...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		triangle(50-TRI_SIZE, 125+TRI_SIZE*2, 50, 125, 50+TRI_SIZE, 125+TRI_SIZE*2);
		...
		line(45, 470, 45+10, 470+10);
		line(45, 470+10, 45+10, 470);
	...
	private void printQuakes() 
	{
		...
		// initiate them here so that you can use them out of the for loop
		int landCount = 0;
		int oceanCount = 0;
		for(Marker cm : countryMarkers) {
			// must reset them to 0 or the count will keep on adding
			landCount = 0;
			oceanCount = 0;
			
			for(Marker q : quakeMarkers) {
				if(cm.getProperty("name") == q.getProperty("country")) 
					landCount++;
				if(q.getProperty("country") == null) 
					oceanCount++;		
			}
			System.out.println(cm.getProperty("name"));
			System.out.println("Land Quakes: " + landCount);
		}
